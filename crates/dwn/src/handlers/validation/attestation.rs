...
pub async fn validate_attestation(did: &Did, msg: &Message) -> Result<(), ValidationError> {
    ...
    for signature in attestation.signatures.iter() {
        ...
        let Some(vc) = document
            .resolve_verification_method(&signature.header.kid, VerificationRole::Assertion)
        else {
            debug!(
        ...
        let signed_payload = header_str + "." + &attestation.payload;
