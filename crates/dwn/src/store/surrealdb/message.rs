    ...
    fn put(
        ...
        Box::pin(async move {
            ...
            if let Some(data) = message.data.take() {
                let db = store.data_db().await.map_err(MessageStoreError::Backend)?;
                                        ...
                                        if let Some(of) = &action.of {
                                            ...
                                            if count < context_cids.len() {
                                                ...
                                                let message = message.ok_or(
                                                    MessageStoreError::NotFound("message"),
                                                )?;
    ...
    fn query_records(
        ...
        Box::pin(async move {
            ...
            if let Some(protocol_version) = filter.protocol_version {
                ...
                conditions
                    .add("message.descriptor.protocolVersion = $protocol_version".to_string());
            }
